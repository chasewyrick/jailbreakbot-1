    ...
    for (var i = 0; i < values.length; i++) {
        ...
        if (val >= 588) pretty = "since a long while ago";
            ...
            pretty +=
                ...
                `${
                    val == 1
                        ? lookup[i].substr(0, lookup[i].length - 1)
                        : lookup[i]
                }` +
                " ago";
        break;
...
module.exports = class SayCommand extends Command {
    ...
    async run(msg, { member }) {
        ...
        if (member.lastMessage)
            var diff3 =
                date.valueOf() - new Date(member.lastMessage.createdTimestamp);
        const embed = new Discord.RichEmbed()
            ...
            .addField(
                ...
                member.presence.status +
                    " " +
                    getRelativeTime(date.valueOf() - parseInt(res)),
                true
            )
            .addField(
                "Created",
                `${date2.customFormat(
                    "#MM#/#DD#/#YYYY# #hh#:#mm#:#ss#"
                )} (${getRelativeTime(diff, 1)})`
            )
            .addField(
                "Joined",
                `${date4.customFormat(
                    "#MM#/#DD#/#YYYY# #hh#:#mm#:#ss#"
                )} (${getRelativeTime(diff2, 1)})`
            );
        if (!member.presence.game)
            embed.addField("Playing", "~~Not in game~~", true);
