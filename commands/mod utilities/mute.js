...
module.exports = class SayCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            memberName: "mute",
            description:
                "Mutes a member for the amount of time it was specified\nContributions by @oliver#9880.",
    ...
    async run(msg, { member, timer, reason }) {
        ...
        const res = await db
            .hgetall(`${member.user.id}mutedInfo`)
            .catch(console.error);
        if (res)
            return msg
                .reply(`${member.user.tag} is already muted.`)
                .catch(console.error);
        ...
        if (type === "s") {
            ...
            duration = `${timer.substr(0, timer.length - 1)} ${
                time == 1 ? "hour" : "hours"
            }`;
            time = time * 3600000;
        } else if (type === "d") {
        ...
        if (!mutedRole)
            return msg
                .reply("I couldn't find the mute role")
                .then((e) => e.delete(3000));
        member
            .addRole(mutedRole)
            .then(async () => {
