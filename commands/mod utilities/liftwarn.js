...
module.exports = class SayCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            memberName: "liftwarn",
            description:
                "Removes the amount of warning points it was specified from a member.",
    ...
    async run(msg, { member, points, reason }) {
        ...
        if (
            ...
            return msg.reply(
                `You do not have the authority to perform moderation actions on ${
                    member.displayName
                }`
            );
        ...
        if (!channel)
            return msg
                .reply("You haven't set a mod logging channel!")
                .then((e) => e.delete(3000));
        const points_before_warn = parseInt(
            await db.hget("warnpoints", member.user.id)
        );
        if (points_before_warn - points <= 0)
            db.hdel("warnpoints", member.user.id);
        ...
        db.incr("cases").then(() => {
            const embed = new Discord.RichEmbed()
                ...
                .setColor(0xa84300)
                .setFooter(
                    `Case #${caseNumber ? caseNumber : 0} | ${member.user.id}`
                )
                .setTimestamp()
            ...
            modlog.send({ embed }).then(() => {
                member.user
                    .send(
                        `You have been de-warned by ${
                            msg.author.tag
                        } for: ${reason}`
                    )
                    .catch(console.error);
                msg.client.guilds
                    .get(config.staff_server)
                    .channels.get(config.staff_mod_logs)
                    .send({ embed })
                    .catch(console.error);
            });
        });
