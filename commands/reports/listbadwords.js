...
module.exports = class SayCommand extends Command {
    ...
    async run(message) {
        ...
        const embed = new Discord.RichEmbed()
            ...
            .setColor(0x00ae86)
            .setDescription(
                "Priority 0: Doesn't ping anyone.\nPriority 1: Pings the Moderators that have choosen to be pinged when they are online.\nPriority 2:Pings Moderators role."
            );
        const bad_words_arr = properties
            .filter((item) => bad_words[item] == 0)
            .join("\n");
        const priority_bad_words = properties
            .filter((item) => bad_words[item] == 1)
            .join("\n");
        const top_priority_bad_words = properties
            .filter((item) => bad_words[item] == 2)
            .join("\n");
        if (bad_words_arr.length)
            embed.addField("Priority 0:", `${bad_words_arr}`);
        if (priority_bad_words.length)
            embed.addField("Priority 1:", `${priority_bad_words}`);
        if (top_priority_bad_words.length)
            embed.addField("Priority 2:", `${top_priority_bad_words}`);
