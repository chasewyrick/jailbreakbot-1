...
module.exports = class SayCommand extends Command {
    ...
    async run(msg, { emoji, role, type, text }) {
        ...
        emoji = staff_server.emojis.get(emoji)
            ? staff_server.emojis.get(emoji)
            : emoji;
        const db_string =
            type === "jailbreak" || type === "jb"
                ? "jailbreak_updates"
                : type === "firmware"
                    ? "firmware_updates"
                    : "other_updates";
        ...
        await db.hmset(
            `${db_string}${isNaN(number) ? 0 : number.toString()}`,
            "role",
            role.id,
            "emoji",
            emoji.id,
        ...
        await msg
            .reply("Saved new requestable role!")
